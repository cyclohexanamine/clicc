...
(defmacro defclass-threaded (class-name parents slots)
  (let ((slot-names (get-slot-names slots)))
   `(progn
      (cl:defclass ,class-name ,parents 
        ,(append slots `((locks
                          :initform (loop for lock-name in (quote ,slot-names)
                                           collecting (cons lock-name (make-lock)))))))
      ,@(loop for slot-name in slot-names
             collecting `(defslotinterface ,class-name ,slot-name ,(mashup-symbol 'read- slot-name) ,(mashup-symbol 'modify- slot-name))))))
